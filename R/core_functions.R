## the following three functions are created to update C
## solving C directly by minimizing the objective function
update_C_direct <- function(M, G_old, beta, eta=1e-16) {
  ...
  C_new = solve(t(G_old) %*% G_old + 
                  ...
                  eta * diag(1, n_cell_types, n_cell_types)) %*%
...
## the following two are for solving G
